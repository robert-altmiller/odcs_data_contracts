...
- The fourth step is to run the Data Contract CLI out of the box (OOB) and user-defined data quality SQL rules.  The OOB rules check to make sure all columns exists, and correct datatypes have been assigned.  User-defined data quality rules are specified using Databricks SQL syntax.  For example, rules can be used to to check and make sure a table has data and no duplicates exist across all rows.
